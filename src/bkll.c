...
{	
	...
	if(param->BK_FormFactor_choice ==1)
	{
	...
	double DmBp_BK = 0.04578;// 1306.2384
	double DmBT_BK = 0.052;// 1306.2384

	double P_p=1.-q2/pow(mB+DmBp_BK,2.);
	double P_T=1.-q2/pow(mB+DmBT_BK,2.);

	double a0f0=param->a0f0_BK_AS_LCSR_Lattice;
	double a1f0=param->a1f0_BK_AS_LCSR_Lattice;
	double a2f0=param->a2f0_BK_AS_LCSR_Lattice;
	double a3f0=param->a3f0_BK_AS_LCSR_Lattice;

	double a0fp=param->a0fp_BK_AS_LCSR_Lattice;
	double a1fp=param->a1fp_BK_AS_LCSR_Lattice;
	double a2fp=param->a2fp_BK_AS_LCSR_Lattice;

	double a0fT=param->a0fT_BK_AS_LCSR_Lattice;
	double a1fT=param->a1fT_BK_AS_LCSR_Lattice;
	double a2fT=param->a2fT_BK_AS_LCSR_Lattice;


	f0=a0f0+a1f0*z_q2+a2f0*pow(z_q2,2.)+a3f0*pow(z_q2,3.);
	fp=(1./P_p)*(a0fp+a1fp*z_q2+a2fp*pow(z_q2,2.)+(-a1fp/3.+2.*a2fp/3.)*pow(z_q2,3.));
	fT=(1./P_T)*(a0fT+a1fT*z_q2+a2fT*pow(z_q2,2.)+(-a1fT/3.+2.*a2fT/3.)*pow(z_q2,3.));

	}


	/********Fit parameters from van Dyk et al 2305.06301***************/
	if(param->BK_FormFactor_choice ==2)
	{
	double a0f0=param->a0fp_BK_GRvDV_BSZ;// a0f0 is the same as a0fp
	double a1f0=param->a1f0_BK_GRvDV_BSZ;
	double a2f0=param->a2f0_BK_GRvDV_BSZ;
	double a0fp=param->a0fp_BK_GRvDV_BSZ;
	double a1fp=param->a1fp_BK_GRvDV_BSZ;
	double a2fp=param->a2fp_BK_GRvDV_BSZ;
	double a0fT=param->a0fT_BK_GRvDV_BSZ;
	double a1fT=param->a1fT_BK_GRvDV_BSZ;
	double a2fT=param->a2fT_BK_GRvDV_BSZ;
	...
	double P_0=1.-q2/pow(mRf0,2.);
	double P_p=1.-q2/pow(mRfp,2.);
	double P_T=1.-q2/pow(mRfT,2.);

	f0=(1./P_0)*( a0f0 + a1f0*(z_q2-z_0) + a2f0*pow(z_q2-z_0,2.) );
	fp=(1./P_p)*( a0fp + a1fp*(z_q2-z_0) + a2fp*pow(z_q2-z_0,2.) );
	fT=(1./P_T)*( a0fT + a1fT*(z_q2-z_0) + a2fT*pow(z_q2-z_0,2.) );


	}


	/********LCSR+Lattice fit parameters from van Dyk et al 1811.00983***************/
	if(param->BK_FormFactor_choice ==3)
	{
	double a0f0=param->a0fp_BK_GKvD_LCSR_Lattice;// a0f0 is the same as a0fp
	double a1f0=param->a1f0_BK_GKvD_LCSR_Lattice;
	double a2f0=param->a2f0_BK_GKvD_LCSR_Lattice;
	double a0fp=param->a0fp_BK_GKvD_LCSR_Lattice;
	double a1fp=param->a1fp_BK_GKvD_LCSR_Lattice;
	double a2fp=param->a2fp_BK_GKvD_LCSR_Lattice;
	double a0fT=param->a0fT_BK_GKvD_LCSR_Lattice;
	double a1fT=param->a1fT_BK_GKvD_LCSR_Lattice;
	double a2fT=param->a2fT_BK_GKvD_LCSR_Lattice;
	...
	/********LCSR-only fit parameters from van Dyk et al 1811.00983***************/
	if(param->BK_FormFactor_choice ==4)
	{
	double a0f0=param->a0fp_BK_GKvD_LCSRonly;// a0f0 is the same as a0fp
	double a1f0=param->a1f0_BK_GKvD_LCSRonly;
	double a2f0=param->a2f0_BK_GKvD_LCSRonly;
	double a0fp=param->a0fp_BK_GKvD_LCSRonly;
	double a1fp=param->a1fp_BK_GKvD_LCSRonly;
	double a2fp=param->a2fp_BK_GKvD_LCSRonly;
	double a0fT=param->a0fT_BK_GKvD_LCSRonly;
	double a1fT=param->a1fT_BK_GKvD_LCSRonly;
	double a2fT=param->a2fT_BK_GKvD_LCSRonly;
	...
	/******** B -> K FF from FLAG24 [2411.04268] ********/
	if(param->BK_FormFactor_choice == 6)
	{
		// mB = 5.27931; // FLAG24 value
		// mK = 0.493677; // FLAG24 value
		double a0fp = param->a0fp_BK_FLAG24;
		double a1fp = param->a1fp_BK_FLAG24;
		double a2fp = param->a2fp_BK_FLAG24;
		double a0f0 = param->a0f0_BK_FLAG24;
		double a1f0 = param->a1f0_BK_FLAG24;
		// a2f0 is given below from the constraint of f0(q2=0)=f0(q2=0)
		double a0fT = param->a0fT_BK_FLAG24;
		double a1fT = param->a1fT_BK_FLAG24;
		double a2fT = param->a2fT_BK_FLAG24;

		double mRfpT = 5.4154;    // Resonance mass for f+ and fT
		double mRf0 = 5.718;    // Resonance mass for f0

		double t0 = (mB + mK) * pow(sqrt(mB) - sqrt(mK), 2.0);
		double tp = pow(mB + mK, 2.);

		double sqrt_tp_q2 = sqrt(tp - q2);
		double sqrt_tp_t0 = sqrt(tp - t0);
		double z_q2 = (sqrt_tp_q2 - sqrt_tp_t0) / (sqrt_tp_q2 + sqrt_tp_t0);

		double sqrt_tp = sqrt(tp);
		double z0 = (sqrt_tp - sqrt_tp_t0) / (sqrt_tp + sqrt_tp_t0);

		// a2f0 using the constraint equation from fp(q2=0)=f0(q2=0)
		double a2f0 = a2fp + (a0fp - a0f0)/pow(z0, 2.) + (a1fp - a1f0)/z0 + (2./3. * a2fp - 1./3. * a1fp) * z0;

		double P_p = 1. - q2 / pow(mRfpT, 2.);
		double P_0 = 1. - q2 / pow(mRf0, 2.);
		double P_T = 1. - q2 / pow(mRfpT, 2.);

		f0 = (1. / P_0) * (a0f0 + a1f0 * z_q2 + a2f0 * pow(z_q2, 2.));
		fp = (1. / P_p) * (a0fp + a1fp * (z_q2 - (1./3.) * pow(z_q2, 3.)) + a2fp * (pow(z_q2, 2.) + (2./3.) * pow(z_q2, 3.)));
		fT = (1. / P_T) * (a0fT + a1fT * z_q2 + a2fT * pow(z_q2, 2.));

	}


	/******** B -> K FF from HPQCD22 [2207.12468] ********/
	if(param->BK_FormFactor_choice == 7)
	{
		double a0f0 = param->a0f0_BK_HPQCD22;
		double a1f0 = param->a1f0_BK_HPQCD22;
		double a2f0 = param->a2f0_BK_HPQCD22;
		double a0fp = a0f0;// ap_0 parameter
		double a1fp = param->a1fp_BK_HPQCD22;
		double a2fp = param->a2fp_BK_HPQCD22;
		double a0fT = param->a0fT_BK_HPQCD22;
		double a1fT = param->a1fT_BK_HPQCD22;
		double a2fT = param->a2fT_BK_HPQCD22;
		double chiralLog = param->L_BK_HPQCD22;  // Chiral Log factor

		double mBsStar = 5.4158;    // Resonance mass for f+ and fT
		double mBsStar0p = 5.729495;  // Resonance mass for f0

		double t0 = 0.;// See text after Eq.12
		double tp = pow(mB + mK, 2.);
		...
		double P_p = 1. - q2 / pow(mBsStar, 2.);
		double P_0 = 1. - q2 / pow(mBsStar0p, 2.);
		double P_T = 1. - q2 / pow(mBsStar, 2.);

		f0 = (chiralLog / P_0) * (a0f0 + a1f0 * z_q2 + a2f0 * pow(z_q2, 2.));
		fp = (chiralLog / P_p) * (a0fp + a1fp * (z_q2 - (1./3.) * pow(z_q2, 3.)) + a2fp * (pow(z_q2, 2.) + (2./3.) * pow(z_q2, 3.)));
		fT = (chiralLog / P_T) * (a0fT + a1fT * (z_q2 - (-1.)* (1./3.) * pow(z_q2, 3.)) + a2fT * (pow(z_q2, 2.) - (-1.)* (2./3.) * pow(z_q2, 3.)));

	}

	/********LCSR-only from Khodjamirian & Rusov [1703.04765] ***************/
	if(param->BK_FormFactor_choice ==8)
	{
	// double f0_0=param->f0fp_BK_KR;// f0f0 is the same as f0fp
	// double f0_b1=param->b1f0_BK_KR;// In KR, result is not given for f0
	double fp_0=param->f0fp_BK_KR;
	double fp_b1=param->b1fp_BK_KR;
	double fT_0=param->f0fT_BK_KR;
	double fT_b1=param->b1fT_BK_KR;
	...
	// double P_0=1.-q2/pow(mRf0,2.);// In KR, result is not given for f0
	double P_p=(1.-q2/pow(mRfp,2.));
	double P_T=(1.-q2/pow(mRfT,2.));

	// f0=(1./P_0)*( a0f0 + a1f0*(z_q2-z_0) + a2f0*pow(z_q2-z_0,2.) );
	fp=fp_0*(1./P_p)*( 1. + fp_b1*(z_q2 - z_0 + 1./2.*(z_q2*z_q2 - z_0*z_0))) ;
	fT=fT_0*(1./P_T)*( 1. + fT_b1*(z_q2 - z_0 + 1./2.*(z_q2*z_q2 - z_0*z_0))) ;
	f0=fp; // This is NO A GOOD APPROXIMATION in the full q2-range - In KR, result is not given for f0!
	}
	...
	{	
		...
		/* Power correction error is not needed for FA, FS, FP */
	...
	if(q2>LOW_q2)
	{
		...
		FA=(C10+C10p)*fp;
		FV=(C9eff+C9p)*fp+2.*param->mass_b/(mB+mK)*(C7eff+C7effp+4.*ml/param->mass_b*CT)*fT;
		FS=(mB*mB-mK*mK)/2./(param->mass_b-param->mass_s)*(CQ1+CQ1p)*f0;
		FP=(mB*mB-mK*mK)/2./(param->mass_b-param->mass_s)*(CQ2+CQ2p)*f0-ml*(C10+C10p)*(fp-(mB*mB-mK*mK)/q2*(f0-fp));
		...
		{
			a_l=a_l*(HIGH_q2-q2)/(HIGH_q2-LOW_q2)+a_l_high*(q2-LOW_q2)/(HIGH_q2-LOW_q2);
			b_l=b_l*(HIGH_q2-q2)/(HIGH_q2-LOW_q2)+b_l_high*(q2-LOW_q2)/(HIGH_q2-LOW_q2);
			c_l=c_l*(HIGH_q2-q2)/(HIGH_q2-LOW_q2)+c_l_high*(q2-LOW_q2)/(HIGH_q2-LOW_q2);
		}
...
{	
	...
	{	
		...
		double FV_err = 1.+param->BtoKlow_FV_err_noq2+q2/6.*param->BtoKlow_FV_err_q2;
		FA=(C10+C10p)*fp;
		FV=(C9+Y+C9p)*fp+2.*(mb+alphas_mub/3./pi*DeltaM)/(mB+mK)*(C7eff+C7effp+4.*ml/mb*CT)*fT+2.*(mb+alphas_mub/3./pi*DeltaM)/(mB+mK)*TauP*FV_err;
	...
	for(ie=0;ie<=2;ie++)
	{
		obs[1][ie]=AFB[ie];
		obs[2][ie]=FH[ie];
		// If you intend to add physical observables, add them above following 2 and change Nobs_BKll_phys in `include.c` accordingly
	}
	/****** Outputting non-physical quantities ******/
	// If you intend to add non-physical observables, add them below after last line:
	obs[Nobs_BKll_phys+1][0] = fp;
	obs[Nobs_BKll_phys+2][0] = f0;
	obs[Nobs_BKll_phys+3][0] = fT;
...
double BRBKll_full(int gen, int charge, double smin, double smax, double obs[], double complex C0b[], double complex C1b[], double complex C2b[], double complex CQ0b[], double complex CQ1b[], double complex Cpb[], double complex CQpb[], struct parameters* param, double mu_b)
{
	...
	double obs_min[Nobs_BKll+1],obs_max[Nobs_BKll+1];
	for(je=0;je<=Nobs_BKll;je++) obs_max[je]=obs_min[je]=0.;
	/*************************************************************/
	...
	obs[2]=0.; /* integrated FH */
	/****** Outputting non-physical quantities ******/
	obs[Nobs_BKll_phys+1]=0.; /* non-integrated (averaged) fp */
	obs[Nobs_BKll_phys+2]=0.; /* non-integrated (averaged) f0 */
	obs[Nobs_BKll_phys+3]=0.; /* non-integrated (averaged) fT */
	/************************************************************/
	...
	/****** Outputting non-physical quantities - 18/04/2025 ******/
	...
	obs[0]=s0;
	for(je=1;je<=Nobs_BKll;je++)
	if(je<=Nobs_BKll_phys)
	{
		obs[je]=obs_num[je]/obs_den[je];
	}
	/****** Outputting non-physical quantities - 19/04/2025 ******/
	else if (je>Nobs_BKll_phys)// Catching the non-physical observables (last lines appearing in `dGamma_BKll_dq2_full` function)
	{
