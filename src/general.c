...
double complex CLi2(double complex x) {
    ...
    // Handle cases for x >= 0.51
    if (creal(x) >= x_2) {
        double complex logx = clog(x);
        double complex log1mx = clog(1. - x);

        // Apply the identity and account for the special condition
        double complex result = pisq6 - CLi2(1. - x) - logx * log1mx;

        // If x is real and greater than 1, negate the imaginary part (to agree with clog branch cut treatment)
        if (creal(x) > 1.0 && cimag(x) == 0.0) {
            return creal(result) + I * (-cimag(result)); // Negate the imaginary part
        }
        return result;
    }

    // Handle case when |x| > 1.2 or large imaginary part
    if ((fabs(cimag(x)) > 1.) || (creal(x) * creal(x) + cimag(x) * cimag(x) > 1.2)) {
        return -CLi2(1. / x) - 0.5 * clog(-x) * clog(-x) - pisq6;
    }

    // Handle case when Re(x) <= -0.30
    if (creal(x) <= x_0) {
        double complex zz = clog(1. - x);
        return -CLi2(-x / (1. - x)) - zz * zz / 2.;
    }

    // Handle small values of x (Re(x) < 0.25)
    else if (creal(x) < x_1) {
    ...
    // Handle intermediate values of x
