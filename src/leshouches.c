...
/* initializes the "param" structure by initializing the parameters with either 0 or a value from the PDG */
{
	...
	/***** DIFFERENT THEORY APPRAOCHES FOR SEMI-LEPTONIC DECAYS ******/
	/* Theory approach for B->K*ll, Bs->phill and B->Kll
	 * 0) Soft form factor approach
	 * 1) Full form factor approach
	 * (DEFAULT:1) */
	param->fullFF=1;
	/*****************************************************************/
	...
	/***** DIFFERENT IMPLEMENTATIONS OF POWER CORRECTIONS FOR B->K*ll ******/
	/* Power corrections implementations
	 * 1) QCDf [0106067 & 0412400] + a guesstimated parameterisation of power corrections
	 * 2) van Dyk et al. [1707.07305]
	 * 3) Khodjamirian et al. (the charm part of Y + soft gluon) - does not contain QCDff
	 * 4) QCDf + hadronic fit for power corrections - does not include any guesstimated power correction
	 *    (appropriate fitted values for hplus0r,...,hzero2i should be given in nuissance.in and nuissance_corr.in)
	 * (DEFAULT:1) */
	...
	/***** DIFFERENT FORM FACTOR PARAMETERISATIONS FOR SEMI-LEPTONIC DECAY ****/
	/* ---------------------------------------------- */
	/* B->Kstar form factor choice:
	 * 1) BSZ LCSR+Lattice [1503.05534]                  -> valid for full range
	 * 2) GRvDV LCSR+Lattice [2305.06301]                -> valid for full range
	 * 3) GKvD LCSR+Lattice [1811.00983](FAULTY RESULTS) -> valid for full range
	 * 4) GKvD LCSR-only [1811.00983]                    -> to be used only at low-q2
	 * 5) BSZ LCSRonly [1503.05534]                      -> to be used only at low-q2
	 * 6) HLMW Lattice-only [1310.3722]+[1501.00367]     -> to be used only at high-q2
	 * (DEFAULT:2) */
	param->BKstar_FormFactor_choice=2;
	/* ---------------------------------------------- */
	 ...
	 * 6) HLMW Lattice-only [1310.3722]+[1501.00367] -> to be used only at high-q2
	 * (DEFAULT:2) */
	param->Bsphi_FormFactor_choice=2;
	/* ---------------------------------------------- */


	/* ---------------------------------------------- */
	/* B->K form factor choice:
	 * 1) AS LCSR+Lattice [1411.3161]       -> valid for full range
	 * 2) GRvDV LCSR+Lattice [2305.06301]   -> valid for full range
	 * 3) GKvD LCSR+Lattice [1811.00983]    -> valid for full range
	 * 4) GKvD LCSR-only [1811.00983]       -> to be used only at low-q2
	 * 5) not available                     -> ---
	 * 6) FLAG24 Lattice [2411.04268]       -> to be used only at high-q2
	 *     (avg. HPQCD13 [1306.2384] & FNAL/MILC [1509.06235])
	 * 7) HPQCD22 Lattice [2207.12468]      -> valid for full range
	 * 8) KR LCSR-only [1703.04765]         -> to be used only at low-q2; should NOT be used for l=tau or NP study involving (pseudo)scalars
	 * (DEFAULT:3) */
	param->BK_FormFactor_choice=3;
	/* ---------------------------------------------- */
	/**************************************************************************/
	...
	/* Other */
	param->log_mu_W_mass_W=param->log_mu_b_mass_b=param->log_mu_spec_lambda_h_mass_b=0.;
	...
	/* Flavour physics */
	param->f_B=0.1900; // FLAG 24 (2411.04268 eq 168)
	param->f_Bs=0.2303;// FLAG 24 (2411.04268 eq 169)
	param->f_Ds=0.2499; // FLAG 24 (2411.04268 eq 130)
	param->f_D=0.2120; // FLAG 24 (2411.04268 eq 129)
	param->f_K=0.1557; // FLAG 24
	param->fK_fpi=1.1934; // FLAG 24
	...
	param->m_Bs=5.36693;
	param->m_Bd=5.27972;
	param->m_pi=0.13957039;
	param->m_pip=0.1349768;
	param->m_K=0.493677;
	param->m_K0=0.497611;
	param->m_Kstar=0.89167;
	param->m_Kstar0=0.89555;
	param->m_D0=1.86484;
	param->m_D=1.86966;
	param->m_Dstar=2.01026;
	param->m_Dstar0=2.00685;
	param->m_Ds=1.96835;
	param->m_phi=1.019461;
	param->life_pi=2.6033e-8;
	param->life_K=1.2380e-8;
	param->life_B=1.638e-12;
	param->life_Bs=1.527e-12;
	param->ys_Bs=0.0635; // 1/2 Delta(Gamma_Bs)/Gamma_Bs  PDG 2024
	param->life_Bd=1.517e-12;
	param->life_D=1.033e-12;
	param->life_Ds=5.012e-13;
	param->a1perp=0.04;
	param->a2perp=0.10;
	param->a1par=0.06;
	param->a2par=0.16;
    ...
    param->a0A0_BKstar_GRvDV_BSZ=0.34213768;
    param->a1A0_BKstar_GRvDV_BSZ=-1.14740932;
    param->a2A0_BKstar_GRvDV_BSZ=2.37275841;
    param->a0A1_BKstar_GRvDV_BSZ=0.28926565;
    param->a1A1_BKstar_GRvDV_BSZ=0.45994786;
    param->a2A1_BKstar_GRvDV_BSZ=1.22417351;
    param->a1A12_BKstar_GRvDV_BSZ=0.5511594;
    param->a2A12_BKstar_GRvDV_BSZ=0.57813167;
    param->a0V_BKstar_GRvDV_BSZ=0.36280141;
    param->a1V_BKstar_GRvDV_BSZ=-1.08931588;
    param->a2V_BKstar_GRvDV_BSZ=2.80292123;
    param->a0T1_BKstar_GRvDV_BSZ=0.31924221;
    param->a1T1_BKstar_GRvDV_BSZ=-0.96188597;
    param->a2T1_BKstar_GRvDV_BSZ=2.09305925;
    param->a1T2_BKstar_GRvDV_BSZ=0.59323779;
    param->a2T2_BKstar_GRvDV_BSZ=1.6795425;
    param->a0T23_BKstar_GRvDV_BSZ=0.62123502;
    param->a1T23_BKstar_GRvDV_BSZ=0.97330702;
    param->a2T23_BKstar_GRvDV_BSZ=1.82128227;


	/* LCSR+Lattice B->Kstar FF van Dyk et al. 1811.00983 (BKstar_FormFactor_choice:3)*/
	param->a0A0_BKstar_GKvD_LCSR_Lattice=0.343392099713454;
	param->a1A0_BKstar_GKvD_LCSR_Lattice=-1.12963120225499;
	param->a2A0_BKstar_GKvD_LCSR_Lattice=2.32629194882251;
	param->a0A1_BKstar_GKvD_LCSR_Lattice=0.301238369699384;
	param->a1A1_BKstar_GKvD_LCSR_Lattice=0.504628070962564;
	param->a2A1_BKstar_GKvD_LCSR_Lattice=1.14175003219003;
	param->a1A12_BKstar_GKvD_LCSR_Lattice=0.555396279655387;
	param->a2A12_BKstar_GKvD_LCSR_Lattice=0.664705390099347;
	param->a0V_BKstar_GKvD_LCSR_Lattice=0.381547116851104;
	param->a1V_BKstar_GKvD_LCSR_Lattice=-1.10351114718558;
	param->a2V_BKstar_GKvD_LCSR_Lattice=2.10663506448104;
	param->a0T1_BKstar_GKvD_LCSR_Lattice=0.337819625135742;
	param->a1T1_BKstar_GKvD_LCSR_Lattice=-0.867784500993516;
	param->a2T1_BKstar_GKvD_LCSR_Lattice=-0.116944346656631;
	// param->a0T2_BKstar_GKvD_LCSR_Lattice=0.337819625135742;
	param->a1T2_BKstar_GKvD_LCSR_Lattice=0.69596963467495;
	param->a2T2_BKstar_GKvD_LCSR_Lattice=-1.11367350668332;
	param->a0T23_BKstar_GKvD_LCSR_Lattice=0.645487422012091;
	param->a1T23_BKstar_GKvD_LCSR_Lattice=0.7287741888333;
	param->a2T23_BKstar_GKvD_LCSR_Lattice=-0.311255376696269;

	/* LCSR-only B->Kstar FF van Dyk et al. 1811.00983 (BKstar_FormFactor_choice:4)*/
	param->a0A0_BKstar_GKvD_LCSRonly=0.305200849074031;
	param->a1A0_BKstar_GKvD_LCSRonly=-0.906213833421311;
	param->a2A0_BKstar_GKvD_LCSRonly=0.618052979733649;
	param->a0A1_BKstar_GKvD_LCSRonly=0.259394534444493;
	param->a1A1_BKstar_GKvD_LCSRonly=0.437308104587007;
	param->a2A1_BKstar_GKvD_LCSRonly=-0.574620717247898;
	param->a0V_BKstar_GKvD_LCSRonly=0.324516018649788;
	param->a1V_BKstar_GKvD_LCSRonly=-0.775925434604412;
	param->a2V_BKstar_GKvD_LCSRonly=-0.561547836543204;
	param->a1A12_BKstar_GKvD_LCSRonly=0.256265611170664;
	param->a2A12_BKstar_GKvD_LCSRonly=-0.55034098071292;
	param->a0T1_BKstar_GKvD_LCSRonly=0.287494870046393;
	param->a1T1_BKstar_GKvD_LCSRonly=-0.720900476495082;
	param->a2T1_BKstar_GKvD_LCSRonly=-0.245776850357086;
	param->a1T2_BKstar_GKvD_LCSRonly=0.594783128831055;
	param->a2T2_BKstar_GKvD_LCSRonly=-0.928219290733931;
	param->a0T23_BKstar_GKvD_LCSRonly=0.579857266209497;
	param->a1T23_BKstar_GKvD_LCSRonly=0.653626191725604;
	param->a2T23_BKstar_GKvD_LCSRonly=-0.450431767839314;

	/* LCSR-only B->Kstar FF Zwicky et al. 1503.05534 (BKstar_FormFactor_choice:5)*/
	param->a0V_BKstar_BSZ_LCSRonly=0.341428;
	param->a1V_BKstar_BSZ_LCSRonly=-1.04834;
	param->a2V_BKstar_BSZ_LCSRonly=2.37143;
	param->a0T1_BKstar_BSZ_LCSRonly=0.28235;
	param->a1T1_BKstar_BSZ_LCSRonly=-0.888396;
	param->a2T1_BKstar_BSZ_LCSRonly=1.94823;
	param->a0T2_BKstar_BSZ_LCSRonly=0.28235;
	param->a1T2_BKstar_BSZ_LCSRonly=0.398974;
	param->a2T2_BKstar_BSZ_LCSRonly=0.36137;
	param->a0T23_BKstar_BSZ_LCSRonly=0.667768;
	param->a1T23_BKstar_BSZ_LCSRonly=1.47676;
	param->a2T23_BKstar_BSZ_LCSRonly=1.92352;
	param->a0A0_BKstar_BSZ_LCSRonly=0.355851;
	param->a1A0_BKstar_BSZ_LCSRonly=-1.04363;
	param->a2A0_BKstar_BSZ_LCSRonly=1.12403;
	param->a0A1_BKstar_BSZ_LCSRonly=0.269264;
	param->a1A1_BKstar_BSZ_LCSRonly=0.304578;
	param->a2A1_BKstar_BSZ_LCSRonly=-0.10662;
	param->a0A12_BKstar_BSZ_LCSRonly=0.255783;
	param->a1A12_BKstar_BSZ_LCSRonly=0.601902;
	param->a2A12_BKstar_BSZ_LCSRonly=0.117626;

	/* Lattice-only B->Kstar FF Wingate et al. 1310.3722 + 1501.00367 (BKstar_FormFactor_choice:6*/
	param->a0V_BKstar_HLMW=0.497528420813;
	param->a1V_BKstar_HLMW=-2.0150648425;
	param->a0A0_BKstar_HLMW=0.502302643141;
	param->a1A0_BKstar_HLMW=-1.60847600112;
	param->a0A1_BKstar_HLMW=0.284803612469;
	param->a1A1_BKstar_HLMW=0.191411584999;
	param->a0A12_BKstar_HLMW=0.219527522577;
	param->a1A12_BKstar_HLMW=0.332374417965;
	param->a0T1_BKstar_HLMW=0.419712711234;
	param->a1T1_BKstar_HLMW=-1.36328797303;
	param->a0T2_BKstar_HLMW=0.279965680646;
	param->a1T2_BKstar_HLMW=0.11706607999;
	param->a0T23_BKstar_HLMW=0.523462757534;
	param->a1T23_BKstar_HLMW=-0.27135970224;
	param->systErr_BKstar_HLMW=0.;
    ...
    param->a0A0_Bsphi_GRvDV_BSZ=0.38000311;
    param->a1A0_Bsphi_GRvDV_BSZ=-1.27092227;
    param->a2A0_Bsphi_GRvDV_BSZ=2.75074752;
    param->a0A1_Bsphi_GRvDV_BSZ=0.30467219;
    param->a1A1_Bsphi_GRvDV_BSZ=0.47300921;
    param->a2A1_Bsphi_GRvDV_BSZ=1.96851627;
    param->a1A12_Bsphi_GRvDV_BSZ=0.32277817;
    param->a2A12_Bsphi_GRvDV_BSZ=-0.64655359;
    param->a0V_Bsphi_GRvDV_BSZ=0.37782957;
    param->a1V_Bsphi_GRvDV_BSZ=-0.89784964;
    param->a2V_Bsphi_GRvDV_BSZ=3.89603141;
    param->a0T1_Bsphi_GRvDV_BSZ=0.33630568;
    param->a1T1_Bsphi_GRvDV_BSZ=-0.76226543;
    param->a2T1_Bsphi_GRvDV_BSZ=0.99707755;
    param->a1T2_Bsphi_GRvDV_BSZ=0.74223041;
    param->a2T2_Bsphi_GRvDV_BSZ=2.00139229;
    param->a0T23_Bsphi_GRvDV_BSZ=0.63570005;
    param->a1T23_Bsphi_GRvDV_BSZ=1.1784008;
    param->a2T23_Bsphi_GRvDV_BSZ=1.79990344;

	/* Bs->phi FF Zwicky et al. 1503.05534 (Bsphi_FormFactor_choice:5)*/
	param->a0V_Bsphi_BSZ_LCSRonly=0.386556;
	param->a1V_Bsphi_BSZ_LCSRonly=-1.02957;
	param->a2V_Bsphi_BSZ_LCSRonly=3.50499;
	param->a0T1_Bsphi_BSZ_LCSRonly=0.309106;
	param->a1T1_Bsphi_BSZ_LCSRonly=-0.867808;
	param->a2T1_Bsphi_BSZ_LCSRonly=2.74535;
	param->a0T2_Bsphi_BSZ_LCSRonly=0.309106;
	param->a1T2_Bsphi_BSZ_LCSRonly=0.58328;
	param->a2T2_Bsphi_BSZ_LCSRonly=0.893065;
	param->a0T23_Bsphi_BSZ_LCSRonly=0.675618;
	param->a1T23_Bsphi_BSZ_LCSRonly=2.11435;
	param->a2T23_Bsphi_BSZ_LCSRonly=4.93525;
	param->a0A0_Bsphi_BSZ_LCSRonly=0.388675;
	param->a1A0_Bsphi_BSZ_LCSRonly=-0.783874;
	param->a2A0_Bsphi_BSZ_LCSRonly=2.41473;
	param->a0A1_Bsphi_BSZ_LCSRonly=0.295607;
	param->a1A1_Bsphi_BSZ_LCSRonly=0.483914;
	param->a2A1_Bsphi_BSZ_LCSRonly=0.291817;
	param->a0A12_Bsphi_BSZ_LCSRonly=0.246357;
	param->a1A12_Bsphi_BSZ_LCSRonly=0.764562;
	param->a2A12_Bsphi_BSZ_LCSRonly=0.711083;

	/* Lattice-only Bs->phi FF Wingate et al. 1310.3722 + 1501.00367 (Bsphi_FormFactor_choice:6*/
	param->a0V_Bsphi_HLMW=0.452535336822;
	param->a1V_Bsphi_HLMW=-2.39909309211;
	param->a0A0_Bsphi_HLMW=0.529352133811;
	param->a1A0_Bsphi_HLMW=-1.59248596874;
	param->a0A1_Bsphi_HLMW=0.282831524425;
	param->a1A1_Bsphi_HLMW=0.123793996986;
	param->a0A12_Bsphi_HLMW=0.20660689485;
	param->a1A12_Bsphi_HLMW=0.476257121291;
	param->a0T1_Bsphi_HLMW=0.401597756404;
	param->a1T1_Bsphi_HLMW=-1.13396307551;
	param->a0T2_Bsphi_HLMW=0.282969214555;
	param->a1T2_Bsphi_HLMW=0.248733391065;
	param->a0T23_Bsphi_HLMW=0.521926043325;
	param->a1T23_Bsphi_HLMW=0.384393588684;
	param->systErr_Bsphi_HLMW=0.;


	/* B->K FF */
	param->hadrerrBK=10.; /* in percent */
	/* B->K FF Altmannshofer, Straub 1411.3161 (BK_FormFactor_choice:1) */
	param-> a0f0_BK_AS_LCSR_Lattice=0.54;
	param-> a1f0_BK_AS_LCSR_Lattice=-1.91;
	param-> a2f0_BK_AS_LCSR_Lattice=1.83;
	param-> a3f0_BK_AS_LCSR_Lattice=-0.02;
	param-> a0fp_BK_AS_LCSR_Lattice=0.43;
	param-> a1fp_BK_AS_LCSR_Lattice=-0.67;
	param-> a2fp_BK_AS_LCSR_Lattice=-1.12;
	param-> a0fT_BK_AS_LCSR_Lattice=0.4;
	param-> a1fT_BK_AS_LCSR_Lattice=-0.53;
	param-> a2fT_BK_AS_LCSR_Lattice=-0.29;

	/* B->K FF van Dyk et al. 2305.06301 (BK_FormFactor_choice:2) */
	param->a0fp_BK_GRvDV_BSZ=0.33196382;
	param->a1fp_BK_GRvDV_BSZ=-0.96403506;
	param->a2fp_BK_GRvDV_BSZ=-0.30273752;
	param->a1f0_BK_GRvDV_BSZ=0.33478906;
	param->a2f0_BK_GRvDV_BSZ=0.00372445;
	param->a0fT_BK_GRvDV_BSZ=0.32361471;
	param->a1fT_BK_GRvDV_BSZ=-1.02911783;
	param->a2fT_BK_GRvDV_BSZ=-0.39954681;

	/* LCSR+Lattice B->K FF van Dyk et al. 1811.00983 (BK_FormFactor_choice:3) */
	param->a0fp_BK_GKvD_LCSR_Lattice=0.329089928546368;
	param->a1fp_BK_GKvD_LCSR_Lattice=-0.866946586742744;
	param->a2fp_BK_GKvD_LCSR_Lattice=0.0060956696733412;
	param->a1f0_BK_GKvD_LCSR_Lattice=0.195117199570376;
	param->a2f0_BK_GKvD_LCSR_Lattice=-0.446126457674043;
	param->a0fT_BK_GKvD_LCSR_Lattice=0.299383149250048;
	param->a1fT_BK_GKvD_LCSR_Lattice=-0.773545682402547;
	param->a2fT_BK_GKvD_LCSR_Lattice=0.0095543751025119;

	/* LCSR-only B->K FF van Dyk et al. 1811.00983 (BK_FormFactor_choice:4) */
	param->a0fp_BK_GKvD_LCSRonly=0.265552872895021;
	param->a1fp_BK_GKvD_LCSRonly=-0.646614080417166;
	param->a2fp_BK_GKvD_LCSRonly=-0.133767782563175;
	param->a1f0_BK_GKvD_LCSRonly=0.384122275897843;
	param->a2f0_BK_GKvD_LCSRonly=-0.662882516309175;
	param->a1fT_BK_GKvD_LCSRonly=-0.650868005090539;
	param->a2fT_BK_GKvD_LCSRonly=0.0999901466869552;
	param->a0fT_BK_GKvD_LCSRonly=0.251019232415893;

	/* Lattice FLAG24 average 2411.04268 of HPQCD'13 and FNAL/MILC'15 (BK_FormFactor_choice:6) */
	param->a0fp_BK_FLAG24=0.471;
	param->a1fp_BK_FLAG24=-0.74;
	param->a2fp_BK_FLAG24=0.32;
	param->a0f0_BK_FLAG24=0.301;
	param->a1f0_BK_FLAG24=0.4;
	param->a0fT_BK_FLAG24=0.455;
	param->a1fT_BK_FLAG24=-1;
	param->a2fT_BK_FLAG24=-0.9;


	/* Lattice full q2-region HPQCD22 2207.12468 (BK_FormFactor_choice:7) */
	param->a0f0_BK_HPQCD22=0.2545;
	param->a1f0_BK_HPQCD22=0.21;
	param->a2f0_BK_HPQCD22=0.02;
	param->a1fp_BK_HPQCD22=-0.71;
	param->a2fp_BK_HPQCD22=0.32;
	param->a0fT_BK_HPQCD22=0.255;
	param->a1fT_BK_HPQCD22=-0.66;
	param->a2fT_BK_HPQCD22=0.36;
	param->L_BK_HPQCD22=1.304;

	/* LCSR-only Khodjamirian & Rusov 1703.04765 (BK_FormFactor_choice:8) */
	param->f0fp_BK_KR=0.395;
	param->b1fp_BK_KR=-1.42 ;
	param->f0fT_BK_KR=0.381;
	param->b1fT_BK_KR=-0.87;
	...
	/* Result from B->K* Hadronic Fit (to be used to check how it impacts predictions) */
	param->hplus0r=param->hminus0r=param->hplus1r=param->hminus1r=param->hplus2r=param->hminus2r=param->hzero0r=param->hzero1r=param->hzero2r=param->hplus0i=param->hminus0i=param->hplus1i=param->hminus1i=param->hplus2i=param->hminus2i=param->hzero0i=param->hzero1i=param->hzero2i=0.;
	...
	param->DeltaC9plus=param->DeltaC9minus=param->DeltaC9zero=0.; /* fits of DeltaC9, but independently for the +, - and 0 amplitudes */	
    ...
    param->m_Lambda = 1.115683;
    param->m_Lambdab = 5.6195;
    param->life_Lb = 1.466e-12;

    param->mpole_fh_LbLll = 5.416;
    param->mpole_g_LbLll = 5.750;

    param->alphaL_LbLll = 0.642;

    param->a0_HO_fplus_LbLll=0.422899;
    param->a1_HO_fplus_LbLll=-1.37281;
    param->a2_HO_fplus_LbLll=1.79720;
    param->a0_HO_fperp_LbLll=0.514815;
    param->a1_HO_fperp_LbLll=-1.47811;
    param->a2_HO_fperp_LbLll=1.24964;
    param->a0_HO_gpp_LbLll=0.352156;
    param->a1_HO_gplus_LbLll=-1.29677;
    param->a2_HO_gplus_LbLll=2.71059;
    param->a1_HO_gperp_LbLll=-1.36073;
    param->a2_HO_gperp_LbLll=2.46213;
    param->a0_HO_hplus_LbLll=0.475314;
    param->a1_HO_hplus_LbLll=-0.883985;
    param->a2_HO_hplus_LbLll=-0.819016;
    param->a0_HO_hperp_LbLll=0.374496;
    param->a1_HO_hperp_LbLll=-0.943936;
    param->a2_HO_hperp_LbLll=1.16058;
    param->a0_HO_htildepp_LbLll=0.325580;
    param->a1_HO_htildeplus_LbLll=-0.960313;
    param->a2_HO_htildeplus_LbLll=2.97798;
    param->a1_HO_htildeperp_LbLll=-0.963386;
    param->a2_HO_htildeperp_LbLll=2.47823;
	
	/* kaons */
	param->deltaPcu_Kppipnunu=0.04;
	param->err_Pc_Xlambda_Kppipnunu=0.;
	param->BR_KLgammagamma_exp=5.47e-4;
	param->Aterm_mu_KLmumu=0.71;
	param->chi_gg_Mrho=5.83;
	param->BR_KSgammagamma_exp=2.63e-6;
	param->Iterm_mu_KSmumu=-2.821;
	param->LDsign_KLmumu=+1;
	param->KC10e=param->KC10pe=param->KCQ1e=param->KCQ1pe=param->KCQ2e=param->KCQ2pe=0.;
	param->KC10mu=param->KC10pmu=param->KCQ1mu=param->KCQ1pmu=param->KCQ2mu=param->KCQ2pmu=0.;
	param->KCLe=param->KCRe=param->KCLmu=param->KCRmu=param->KCLtau=param->KCRtau=0.;
	param->KC9e=param->KC9mu=param->KC9tau=0.;
	param->KC9pe=param->KC9pmu=param->KC9ptau=0.;
	param->sign_KLpill=1;
	param->log_muK_1GeV=0.;
	param->KLpill_Ce_Cdir = 4.62;
	param->KLpill_Cmu_Cdir = 1.09;
	param->KLpill_Ce_Cint = 11.3;
	param->KLpill_Cmu_Cint = 2.63;
	param->KLpill_Ce_Cmix = 14.5;
	param->KLpill_Cmu_Cmix = 3.36;
	param->KLpill_Cmu_CPC = 5.2;
	param->KLpill_abs_aS = 1.2;
	param->KLpill_Ce_PA = 1.9e-5;
	param->KLpill_Ce_PP = 3.8e-7;
	param->KLpill_Ce_Sgg = 1.5e-4;
	param->KLpill_Ce_SS = 3.9e-7;
	param->KLpill_Cmu_PA = 2.6e-1;
	param->KLpill_Cmu_PP = 8.5e-3;
	param->KLpill_Cmu_Sgg = 4.0e-2;
	param->KLpill_Cmu_SS = 4.1e-3;
	param->KLpill_AFB_a1pi_gg_7V = 1.3;
	param->KLpill_AFB_a1pi_gg_aS = 1.7;


	/* CKM matrix with Wolfenstein parametrisation from PDG 2024 (CKMfitter method) */
	/* https://pdg.lbl.gov/2024/reviews/rpp2024-rev-ckm-matrix.pdf, eq.12.26 */
	param->CKM_lambda=0.22501; /* +/- 0.00068 */
	param->CKM_A=0.826; /* +0.016 -0.015 */
	param->CKM_rhobar=0.1591; /* +/- 0.0094 */
	param->CKM_etabar=0.3523; /* +0.0073 -0.0071 */


	/* masses and couplings from PDGlive 2025 */
	param->mass_u = 2.16e-3;
	param->mass_d = 4.70e-3;
	param->mass_s = 0.0935;
	param->mass_c = 1.2730;
	param->scheme_c_mass = -1;
	param->mass_b = 4.183;
	param->mass_top_pole = 172.57;

	param->mass_e = 0.510998995000e-3;
	param->mass_mu= 0.1056583755;
	param->mass_tau_pole=1.77693;
	param->mass_tau=param->mass_tau_pole;
	
	param->mass_Z=91.1880;
	param->alphas_MZ=0.1180;
	param->mass_W=80.3692;

	param->mass_h0=param->mass_h0SM=125.20;

	param->gp=param->gp_Q=3.57458e-1;
	param->g2=param->g2_Q=6.51908e-1;
	param->inv_alpha_em=127.930;/* +/- 0.008 */ /* https://pdg.lbl.gov/2024/reviews/rpp2024-rev-standard-model.pdf#page=4 */
