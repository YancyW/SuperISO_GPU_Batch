...
int thdmc_phys(double mh, double mH, double mA, double mHp, double sba, double l6, double l7, double m12_2, double tanb, int type, char name[])
/* generates a LHA file for a 2HDM parameter space point using 2HDMC */
{
	char tmp_char[500];
	char *dir;
	...
	sprintf(tmp_char,"./CalcPhys %.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %.5e %d %s/%s > 2hdmc_%s",mh,mH,mA,mHp,sba,l6,l7,m12_2,tanb,type,dir,name,name);
	system(tmp_char);
