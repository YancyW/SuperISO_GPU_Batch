...
/************************************************************************/
/***************** Minimization routines for Direct Detection ***********/
/************************************************************************/
...
/* finds the minimum (option="min") or maximum (option="max") with a precions ftol of the function func(double x[], double xtra[]) having n free parameters x[] with a lower bound xlimmin and an upper bound xlimmax. The minimum (maximum) value of the function is saved in fmin and the corresponding values of the free parameters in xmin. xtra[] is an array containing the fixed parameters of function func. */
{
	int nmax=5; // maximum number of free parameters.
...
/* finds the minimum (factor=1) or maximum (factor=-1) using a Powell method with a precions ftol of the function func(double x[], double xtra[]) having n free parameters x[] with a lower bound xlimmin and an upper bound xlimmax. The minimum value of the function is saved in fmin and the corresponding values of the free parameters in xmin. xtra[] is an array containing the fixed parameters of function func. The computation starts with the values x0[] of the free parameters x[]. */
{	
	double TINY=1.0e-70;
	int ITMAX=500;
	int nmax=5;
	...
	for(j=0;j<n;j++) xmin[j]=x0[j]; // initializes minimum position
	...
	*fmin=factor*func(x0,xtra);
	...
	for(iter=0;iter<=ITMAX;iter++)
	{
		...
		for (j=0;j<n;j++) // check if it is worth to keep the mean deplacement vector
		{
...
/* finds values  of a0, b0 anf x0 required in function DDbrentmethod1D */
{
	int BRAKETMAX=50;
	int nmax=5;
...
/************************************************************************/
/***************** Integration routines for Direct Detection ***********/
/************************************************************************/
...
/* Computes the integral of function func(x, xtra, DDparam)  for x between a and b and a precision err */
{
	int n=4;
...
/* calculates the integral of func between x= x[n1] and x[n2] recursively, until a precision err is reached, using tabulated values of func f[] */
{
	int Nmax=500;
	int i;
...
/************************************************************************/
/************* Integration routines for Indirect Detection **************/
/************************************************************************/
...
/* find values  of a0, b0 anf x0 required in function IDbrentmethod1D */
{
	int BRAKETMAX=50;
	int nmax=5;
...
double pvalue_fromchi2(int ie, int je)
/* Chi-squared distribution table
   1sigma->pvalue=0.317, 2sigma->pvalue=0.0455, 3sigma->pvalue=0.0027, 4sigma->pvalue=6e-5, 5sigma->pvalue=3e-7 */
{
	const double pvalue_chi2[10][11]={{0.317,1.0,2.30,3.53,4.72,5.89,7.04,8.18,9.30,10.42,11.54},
	...
	return pvalue_chi2[ie][je];
}
		    
int find_isigma(char* sigma)
/* finds the index corresponding to element sigma in the p-value table */
{
	int i;
	if(strcmp(sigma,"1sigma")==0) i=0;
	else if(strcmp(sigma,"80CL")==0) i=1;
	else if(strcmp(sigma,"90CL")==0) i=2;
	else if(strcmp(sigma,"95CL")==0) i=3;
	else if(strcmp(sigma,"2sigma")==0) i=4;
	else if(strcmp(sigma,"99CL")==0) i=5;
	else if(strcmp(sigma,"3sigma")==0) i=6;
	else if(strcmp(sigma,"99.9CL")==0) i=7;
	else if(strcmp(sigma,"4sigma")==0) i=8;
	else if(strcmp(sigma,"5sigma")==0) i=9;
...
/* tests if the option given in input is "conservative", "standard" or "stringent" and print an error message if not */
{
	if(strcmp(option,"standard")==0) return 0;
	else if(strcmp(option,"conservative")==0) return -1;
	else if(strcmp(option,"stringent")==0) return 1;
	else
	{
		printf("Option must be \"conservative\", \"standard\", or \"stringent\"\n");
		return -10;
	}
}
