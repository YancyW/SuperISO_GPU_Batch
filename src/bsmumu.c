...
/* computes the branching ratio of Bs -> l+ l- */
{
	...
	printf("CA=C10*(-sw2/2)=%.5f\t\n",creal(C10)*(-sw2/2.));
#endif	
		
	double inv_alpha_em_mub = 132.;
	double eta_BBS = 0.995;
	for(int ie=0; ie<10; ie++) {
		double scale = 10 - ie;
	}


	return param->Gfermi*param->Gfermi/inv_alpha_em_mub/inv_alpha_em_mub/*/param->inv_alpha_em/param->inv_alpha_em*/*pow(param->m_Bs,3.)*param->f_Bs*param->f_Bs*param->life_Bs/hbar/64./pi/pi/pi*pow(cabs(param->Vtb*conj(param->Vts)),2.)*sqrt(1.-4.*ml*ml/param->m_Bs/param->m_Bs)
	*((1.-4.*ml*ml/param->m_Bs/param->m_Bs)*pow(param->m_Bs/(param->mass_b_pole+param->mass_s)*cabs(CQ1-CQp1),2.) + pow(cabs(param->m_Bs/(param->mass_b_pole+param->mass_s)*(CQ2-CQp2)+2.*(C10-C10p)*ml/param->m_Bs),2.))*eta_BBS;
}
...
/* computes the untagged branching ratio of Bs -> l+ l- */
{
	...
	// printf("Factor muliplying BR to get untagged: %.5f\n",(1.+A_Dgamma*ys)/(1.-ys*ys));
...
/* computes the branching ratio of Bd -> l+ l- */
{
	...
	return param->Gfermi*param->Gfermi/inv_alpha_em_mub/inv_alpha_em_mub/*param->inv_alpha_em/param->inv_alpha_em*/*pow(param->m_Bd,3.)*param->f_B*param->f_B*param->life_Bd/hbar/64./pi/pi/pi*pow(cabs(param->Vtb*conj(param->Vtd)),2.)*sqrt(1.-4.*ml*ml/param->m_Bd/param->m_Bd)
	*((1.-4.*ml*ml/param->m_Bd/param->m_Bd)*pow(param->m_Bd/(param->mass_b_pole+param->mass_d)*cabs(CQ1-CQp1),2.) + pow(cabs(param->m_Bd/(param->mass_b_pole+param->mass_d)*(CQ2-CQp2)+2.*(C10-C10p)*ml/param->m_Bd),2.))*eta_BBS;
