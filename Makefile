...
CC = gcc
DEFINES = -DVERSION=\"$(VERSION)\" -DYEAR=$(YEAR)
CFLAGS = -O3 -pipe -fomit-frame-pointer $(DEFINES)
CFLAGS_MP = 
#CFLAGS_MP = -fopenmp
...
MAKE = make
MAKE_MP=
#MAKE_MP= -j$(shell nproc)

AR = ar
# ARFLAGS := rcsU
# Check if ar supports the -U option
AR_VERSION := $(shell ar --version 2>/dev/null)
ifeq ($(findstring GNU ar,$(AR_VERSION)),GNU ar)
    ARFLAGS := rcsU
else
    ARFLAGS := rcs
endif
...
save:
	...
	cp -p modelindep_chi2.c superiso_$(VERSION)/;\
	cp -p Makefile superiso_$(VERSION)/;\
	cp -rp chi2_input superiso_$(VERSION)/;\
